...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx1, dW1, db1 = affine_backward(dz1, cache1)
        dW1 += self.reg * W1
        dW2 += self.reg * W2
        grads["W1"] = dW1
        grads["b1"] = db1
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #     hdrop, cache = dropout_forward(hidden['h0'], self.dropout_param)
        #     hidden['hdrop0'], hidden['cache_do0'] = hdrop, cache

        input_var = X.copy()
        for i in range(self.L):
            if i + 1 != self.L:

                h, W, b = hidden['h' + str(i)], self.params['W' + str(i + 1)], self.params['b' + str(i + 1)]
                ...
                if self.use_batchnorm:
                    ...
                    hidden['cache_bn' + str(i + 1)] = cache

                out, cache = relu_forward(out)
                hidden['cache_rl' + str(i + 1)] = cache

                if self.use_dropout:
                    out, cache = dropout_forward(out, self.dropout_param)
                    hidden['cache_do' + str(i + 1)] = cache

                hidden['h' + str(i + 1)] = out
        ...
        dh, dW, db = affine_backward(dscores, hidden['cache' + str(self.L)])
        grads["W" + str(self.L)] = dW
        grads["b" + str(self.L)] = db

        for j in range(self.L - 1, 0, -1):

            if self.use_dropout:
                dh = dropout_backward(dh, hidden['cache_do' + str(j)])

            dh = relu_backward(dh, hidden['cache_rl' + str(j)])

            if self.use_batchnorm:
                dh, dgamma, dbeta = batchnorm_backward(dh, hidden['cache_bn' + str(j)])
                grads["gamma" + str(j)] = dgamma
                grads["beta" + str(j)] = dbeta

            dh, dW, db = affine_backward(dh, hidden['cache' + str(j)])
            grads["W" + str(j)] = dW
            grads["b" + str(j)] = db

        for k in range(self.L):
            grads["W" + str(k + 1)] += self.reg * self.params["W" + str(k + 1)]
